...
Navigation::Navigation(std::string objPath,int boxSize)
{
    ...
    if(pcl::io::loadPCDFile<pcl::PointXYZ>(objPath,*cloud)==-1)
    {
        throw 1;
    ...
    else
    {
        ...
        _boxSize=boxSize;
        _minCostCountZ=-99;
        //pcl::io::savePCDFileASCII("/tmp/original.pcd", *_cloud_original);
    }
...
{
    //----使用ConditionalRemoval移除水平面点,保留垂直面点云----
...
void Navigation::cutByVertical()
{
    for (int i = 1; i < _sizeZ+1; ++i)
    {
        ...
        pt.setFilterFieldName("z");//设置过滤时所需要点云类型的z字段
        //设置在过滤字段上的范围
        if (i==_sizeZ)
        {
        ...
        //----模型投影滤波----
        //定义模型系数对象并填充对应数据，使用一个ax+by+cz+d=0的平面模型，a=b=d=0,c=1,也就是X-Y平面
        pcl::ModelCoefficients::Ptr coefficients(new pcl::ModelCoefficients());
        coefficients->values.resize(4);
        coefficients->values[0]=coefficients->values[1]=coefficients->values[3]=0;
        coefficients->values[2]=1;

        pcl::ProjectInliers<pcl::PointXYZRGB> proj;//创建投影对象
        proj.setModelType(pcl::SACMODEL_PLANE);//设置对象对应的投影模型
        proj.setInputCloud(tempCloud);//设置输入点云
        proj.setModelCoefficients(coefficients);//设置模型对应的系数
        proj.filter(*tempCloud);//执行投影滤波存储结果到tempCloud
