...
public class TangoPointCloud : MonoBehaviour, ITangoPointCloud
{
    ...
    /// </summary>
    private const int MAX_POINT_COUNT = 51440;
    ...
    void link()
    {
        ...
        port = new IPEndPoint(IPAddress.Parse(IP), 6000);
    }

    public void changeSendPointCloud()
    {
        if(sendPointCloud)
        {
            sendPointCloud = false;
            isSendText.text = "disconnect";
        }
        else
        {
            sendPointCloud = true;
            isSendText.text = "in transmitting";
        }
    }
    ...
    public void OnTangoPointCloudAvailable(TangoPointCloudData pointCloud)
    {
        ...
        if (m_pointsCount > 0)
        {
            ...
            for (int i = 0; i < m_pointsCount; ++i)
            {
                ...
                //手机扫描点云数量过多，无法一次性传输。所以选择传输部分，即   1/30
                //数据传输使用udp
                //============================================================================================
                if (i % 30 == 0)
                {
                    sber.Append(string.Format("v {0} {1} {2}\n", m_points[i].x, m_points[i].y, -m_points[i].z));
                }
                //============================================================================================
            ...
            //是否需要发送数据
            if(sendPointCloud)
                client.SendTo(Encoding.UTF8.GetBytes(sber.ToString()), port);
